...
def get_bb_everyday():
    global bb_everyday
    bb_everyday = user.get_repo('%s/bb_everyday' % user.get_user().login)
...
def bundle_summary_section():
    ...
    total_label_count = bb_everyday.get_labels().totalCount
    total_issue_count = bb_everyday.get_issues().totalCount
    labels_html_url = 'https://github.com/%s/bb_everyday/labels' % user.get_user().login
...
def bundle_new_created_section():
    ...
    new_5_created_issues = bb_everyday.get_issues()[:5]
...
def bundle_list_by_labels_section():
    ...
    for label in all_labels:
        ...
        # 所有的issue数量, 之后再优化.
        count = 0
        issues_in_label = bb_everyday.get_issues(labels=(label,))
