...
def get_bb-everyday():
    global bb-everyday
    bb-everyday = user.get_repo('%s/bb-everyday' % user.get_user().login)
...
def bundle_summary_section():
    ...
    total_label_count = bb-everyday.get_labels().totalCount
    total_issue_count = bb-everyday.get_issues().totalCount
    labels_html_url = 'https://github.com/%s/bb-everyday/labels' % user.get_user().login
    issues_html_url = 'https://github.com/%s/bb-everyday/issues' % user.get_user().login
...
def bundle_pinned_issues_section():
    ...
    pinned_label = bb-everyday.get_label(':+1:置顶')
    pinned_issues = bb-everyday.get_issues(labels=(pinned_label,))
...
def format_issue_with_labels(issue: Issue):
    ...
    for label in labels:
        labels_str += sub('[%s](https://github.com/%s/bb-everyday/labels/%s)\t|\t' % (
...
def bundle_new_created_section():
    ...
    new_5_created_issues = bb-everyday.get_issues()[:5]
...
def bundle_list_by_labels_section():
    ...
    all_labels = bb-everyday.get_labels()
    ...
    for label in all_labels:
        ...
        # 所有的issue数量, 之后再优化.
        count = 0
        issues_in_label = bb-everyday.get_issues(labels=(label,))
